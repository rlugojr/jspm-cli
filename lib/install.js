...
exports.install = function(targets, options) {
  ...
  .then(function() {
    ...
    return Promise.all(Object.keys(targets).map(function(name) {
      ...
      if (bulk && !config.pjson.dependencies[name]) {
...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    var existing = installed.baseMap[name] || installing.baseMap[name];

    // when no existing version, do a normal lookup thing
    if (!existing)
      return;

    // if the existing version matches the peer dependency expectation or we're in a lock install
    // then use what we have, and retain the original target range if there was one
    if (options.lock || existing.name == located.name && semver.match(located.version, existing.version)) {
      target = config.pjson.dependencies[name] || config.pjson.peerDependencies[name] || config.pjson.devDependencies[name] || located;
      return existing;
    }

    // conflict resolution!
    ui.log('info', '\nPeer dependency conflict for %' + name + '%:\n' +
        '  Currently installed as `' + existing.exactName + '`.\n' +
        '  Package `' + options.parent + '` requires `' + located.exactName + '`.');
    return ui.confirm('Would you like to install %' + name + '% to the new install range?', true)
    .then(function(useNew) {
      ...
      return ui.confirm('Would you like to retain the existing resolution?', true)
      .then(function(useExisting) {
        if (useExisting) {
          target = config.pjson.dependencies[name] || config.pjson.peerDependencies[name] || config.pjson.devDependencies[name] || existing;
          return existing;
        }

        return ui.input('Enter a custom package resolution range')
  ...
  .then(function() {
    ...
    config.loaderBrowser.ensureRegistry(resolution.registry, options.inject);
  ...
  function storeResolution() {
    ...
    if (!options.parent || options.peer) {
      ...
      // if a peer, but primary install exists, we save as a primary install
      if (options.parent && options.peer && config.pjson.dependencies[name])
        options.peer = false;
      if (!options.parent && !options.peer && !options.dev && config.pjson.peerDependencies[name])
        options.peer = true;
  ...
  function downloadPeerDeps(depMap) {
    return downloadDeps(depMap, true);
  }
  ...
  function downloadDeps(depMap, isPeer) {
    dependencyDownloads = (dependencyDownloads || Promise.resolve()).then(function() {
      return Promise.all(Object.keys(depMap).map(function(dep) {
        // we store seen deps to avoid duplications
        if ((isPeer ? seenPeerDeps : seenDeps).indexOf(dep) != -1)
          return;
        (isPeer ? seenPeerDeps : seenDeps).push(dep);
...
function logInstall(name, target, resolution, linked, options) {
  ...
  if (options.parent && !options.peer)
    ...
    ui.log('ok', verb + '%' + name + '% ' + 'to `' + target.exactName + '` (' + actual + ')');
}
...
function visitForkRanges(tree, resolution, name, parentName, visit) {
  ...
  Object.keys(tree.depMap).forEach(function(parent) {
    ...
    Object.keys(curDepMap).forEach(function(dep) {
      ...
      if (!ranges || !ranges[dep])
        return;

      visit(dep, parent, secondary, ranges[dep]);
    });
  });
}
